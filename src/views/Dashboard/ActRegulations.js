...
function ActRegulations() {
  ...
  const fetchRegulations = async () => {
    ...
    try {
      ...
      showErrorToast(toast, error, { title: "Failed to load Act & Regulations" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEditRegulation = (item) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (currentView === "edit" && editingRegulation) {
        await updateActRegulation(editingRegulation._id, payload);
        toast({
          title: "Success",
          description: "Act & Regulation updated successfully",
          status: "success",
          duration: 3000,
          isClosable: true,
        });
      } else {
        await createActRegulation(payload);
        toast({
          title: "Success",
          description: "Act & Regulation created successfully",
          status: "success",
          duration: 3000,
          isClosable: true,
        });
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteActRegulation(deleteTarget);
      toast({
        title: "Deleted",
        description: "Act & Regulation deleted successfully",
        status: "success",
        duration: 2500,
        isClosable: true,
      });
            ...
            <Box as="form" onSubmit={handleSubmit}>
              <SimpleGrid columns={{ base: 1, md: 1 }} spacing={4} mb={4}>
                <FormControl isRequired>
                  ...
                  <Input
                    name="title"
                    placeholder="Enter act / regulation title (e.g. Electricity Act 2003)"
                ...
                <FormControl>
                  ...
                  {currentView === "edit" && editingRegulation?.docUrl && (
              ...
              >
                {currentView === "add" ? "Create Act & Regulation" : "Update Act & Regulation"}
              </Button>
            </Box>
          </CardBody>
        </Card>
    ...
    <Flex flexDirection="column" pt={{ base: "120px", md: "75px" }} h="calc(100vh - 20px)">
          ...
          >
            <Box>
              <Text fontSize={{ base: "lg", md: "xl" }} color={textColor} fontWeight="bold">
                Act & Regulations Management
              </Text>
              <Text fontSize="xs" color="gray.500">
                Manage all official Act and Regulation records
              </Text>
            </Box>
                        ...
                        <HStack spacing={1} justify="center">
                          <Tooltip label="Edit" hasArrow>
                            <Button
                              ...
                              colorScheme="blue"
                              size="sm"
                              onClick={() => handleEditRegulation(item)}
                            >
                              <Icon as={FaEdit} />
                            </Button>
                          </Tooltip>
                      ...
                      <Td colSpan={5} textAlign="center" py={12}>
                        <Flex direction="column" align="center" justify="center" gap={2}>
                          ...
                          <Text color="gray.500" fontWeight="500">
                            {searchQuery ? "No matching Act & Regulations found." : "No Act & Regulations available."}
                          </Text>
                        </Flex>
                      </Td>
                    </Tr>
              ...
              >
                ...
                >
                  ...
                  <Text fontSize="xs" color="gray.600" ml={3} whiteSpace="nowrap">
                    Showing {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, filteredRegulations.length)} of {filteredRegulations.length}
                  </Text>
                </Flex>
      ...
      <Modal isOpen={isDeleteModalOpen} onClose={closeDeleteModal} isCentered>
        ...
        <ModalContent borderRadius="xl" mx={4}>
          ...
          <ModalBody>
            <Text color="gray.600">
              Are you sure you want to delete this Act & Regulation document? This action cannot be undone.
            </Text>
          </ModalBody>
          <ModalFooter gap={3}>
            <Button
